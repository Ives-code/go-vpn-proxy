...
Use Go with sing-box as the outbound protocol engine. Reuse narrowly selected MIT-licensed parsing, builder, and pool concepts from `jasonwong1991/easy_proxies`, retaining license and attribution. The first release registers VLESS, AnyTLS, Hysteria2, and Trojan (the supplied working subscriptions' protocols) plus HTTP upstream proxies, avoiding unrelated protocol modules and their attack surface; additional protocols require an explicit, tested registry expansion.
